...
Yes. Archived versions import their MDX from `versions/<label>/...` and keep their original file layout. The current version can freely restructure `pages/` and the version switcher will gracefully fall back when a path no longer exists in older snapshots.
